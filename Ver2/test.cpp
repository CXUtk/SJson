...
void expect_parse_throw_error(const std::string& text, const char* file, int line, const char* name) {
    ...
    fprintf(stderr, "%s:%d: expect to throw %s, but none was thrown: %s\n", typeid(T).name(), file, line, name);
}

std::shared_ptr<SJson::JsonNode> expect_parse_nothrow(const std::string& text, const char* file, int line) {
    test_count++;
    std::shared_ptr<SJson::JsonNode> ptr;
    try {
        ptr = SJson::JsonConvert::Parse(text);
    }
    catch (std::exception e) {
...
template<>
void expect_parse_value(const std::string& text, const char* file, int line, int expected) {
    test_count++;
    try {
        auto node = SJson::JsonConvert::Parse(text);
...
//    EXPECT_NODE_TYPE(node->ElementAt(0), SJson::ValueType::Null);
//    EXPECT_NODE_TYPE(node->ElementAt(1), SJson::ValueType::Boolean);
//    EXPECT_NODE_TYPE(node->ElementAt(2), SJson::ValueType::Boolean);
//    EXPECT_NODE_TYPE(node->ElementAt(3), SJson::ValueType::Integer);
//    EXPECT_NODE_TYPE(node->ElementAt(4), SJson::ValueType::String);
//
...
//    EXPECT_NODE_TYPE(node, SJson::ValueType::Object);
//    EXPECT_NODE_TYPE(node->GetMember("Obj"), SJson::ValueType::Object);
//    EXPECT_NODE_TYPE(node->GetMember("Arr"), SJson::ValueType::Array);
//    EXPECT_NODE_TYPE(node->GetMember(""), SJson::ValueType::Integer);
//
//    //SJsonErrorCode code;
...
int main()
{
    ...
    try
    {
        auto node = SJson::JsonConvert::Parse("{Age: 21, Company: \"Microsoft\", Name: \"DXTsT\", Object: [1, 2, 3], Test: [1, 2, \"123\", false]}");
    }
